<template>
  <qt-column class="qt-collapse-menu-item-root" :focusable="false">
    <qt-row class="qt-collapse-menu-item-title-root">
      <span class="qt-collapse-menu-item-title" :style="{ opacity: isExpand ? 1 : 0.7 }">解码</span>
    </qt-row>
    <qt-animation ref="animationRef" class="qt-collapse-menu-animation-view-css">
      <div class="qt-collapse-menu-item-content">
        <qt-button text="自动" class="qt-collapse-menu-item-button" />
        <qt-button text="硬解码" class="qt-collapse-menu-item-button" />
        <qt-button text="软解码" class="qt-collapse-menu-item-button" />
      </div>
    </qt-animation>
  </qt-column>
</template>
...
export default defineComponent({
  name: 'qt-collapse-menu-item-four',
  emits: [],
  setup(props, context) {
    ...
    function onCollapseItemExpand(value: boolean) {
      console.log('---------onCollapseItemExpand------four------>>>', value)
