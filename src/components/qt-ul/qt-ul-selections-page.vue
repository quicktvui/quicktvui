  ...
  <div class="es-sdk-root-css" :clipChildren="false">
    ...
    <qt-view class="qt-ul-selections-root-page" :clipChildren="true">
      <div style="margin-top: 36px;">
        <span style="color: black;font-size: 36px;margin-left:36px;margin-bottom: 20px" >qt-ul</span>
        <qt-ul class="qt-ul-root"
               :stableItemSize="100"
          ...
          <template #item="{item,index}">
            <div class="qt-ul-item-div" name="qtUlItem"
                 @click="itemClick($event,index,item)"
                 v-if="item.type === 1" :focusable="true">
              <qt-text class="qt-ul-item-text-demo" duplicateParentState
                       :textSize="40" gravity="center" :text="item?.text"/>
            </div>
          </template>
        </qt-ul>
        <qt-ul class="qt-ul-group"
               :horizontal="true" :clipChildren="false"
               :enablePlaceholder="false"
               :stableItemSize="180"
               :autoSelectPosition="initGroupPosition"
               :singleSelectPosition="initGroupPosition"

               :items="groupList" padding="0,20,0,0">
          <template #item="{item,index}">
            <div class="qt-ul-group-item" v-if="item.type === 1"
                 name="groupItem"
                 @focus="itemFocus($event,index,item)"
                 :focusable="true">
              <qt-text class="qt-ul-group-text"
                       duplicateParentState
                       :textSize="36" gravity="center" :text="item?.text"/>
            </div>
          </template>
        </qt-ul>
      </div>
      <div style="margin-top: 36px;" :focusable="false">
        <span style="color: black;font-size: 36px;margin-left:36px;margin-bottom: 20px" :focusable="false" >qt-list-view</span>
        <qt-list-view class="qt-ul-root"
                      :horizontal="true"
                      padding="0,10,0,0"
                      :focusable="false"
                      ref="qtListViewRef" >
          <div class="qt-ul-item-div" :type="1"  :focusable="false" eventFocus>
            <qt-text class="qt-ul-item-text-demo" duplicateParentState :focusable="false"
                     :textSize="40" gravity="center" text="${text}"/>
          </div>
        </qt-list-view>
