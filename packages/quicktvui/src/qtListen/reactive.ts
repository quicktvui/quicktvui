...
const getReactiveConfig = (root?: any, paths: any[] = [], __qt_arr_deth = 1): ProxyHandler<any> => {
  return {
    get(target, prop, receiver) {
      ...
      if (isObject(result)) {
        const res = createReactiveObject(
          result,
          root,
          [...paths, prop],
          isArray(result) ? __qt_arr_deth + 1 : __qt_arr_deth
        )
        ...
        if (isTrackProp(target, prop)) {
          if (oldValue != value) {
            if (
              ...
              qtType.checkArrChangeProps(target, prop)
            ) {
              value.__qt_change_num_++
            }
            ...
            if (_path && _path.length && !isReactive(value)) {
              ...
              qtType.changeOfsetType(
                _root,
                _path,
                _firstObj,
                _prop,
                receiver[emReactiveFlags.qtArrDeth]
              )
              ...
              if (!isCollect) {
                qtType.changeOfsetType(target, prop, value)
              }
