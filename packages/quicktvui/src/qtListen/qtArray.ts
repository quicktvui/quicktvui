...
class QtArray extends Array {
  ...
  unshift(...items: any[]) {
    if (items.length) {
      ...
      if (!changeData) {
        ...
        // @ts-ignore
        qtType
          .getFlag(this.__v_raw)
          .set(typeEnum.expectCangeNum, this.__v_raw.length + items.length)
      } else {
  ...
  splice(start: number, deleteCount: number, ...items: any[]) {
    ...
    // @ts-ignore
    if (start >= this.__v_raw.length || deleteCount < 0) {
      //防止索引越界
      deleteCount = 0
    }
    ...
    if (!changeData) {
      ...
      qtType
        .getFlag(this.__v_raw)
        .set(typeEnum.expectCangeNum, this.__v_raw.length + items.length - deleteCount - start)
    }
    if (deleteCount <= 0) {
