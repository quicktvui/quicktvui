...
export function qtWatchAll(target: any, options: IQtWatchOptions) {
  ...
  const _effect = new QtReactiveEffect(
    ...
    (target, prop, newValue, type, oldValue) => {
      ...
      if (
        type === emReactiveFlags.RESET_REF_VALUE &&
        !(
          _currentType === typeEnum.concat &&
          oldTarget &&
          (oldTarget.length || Object.keys(newValue).length)
        )
      ) {
        ...
        _currentType === typeEnum.splice &&
        !(target && (target.length || Object.keys(newValue).length))
      ) {
        options.clear()
        __v_raw = target
