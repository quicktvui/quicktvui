 ...
 * 解析子列表更新层级
 */
export const parseChildUpdate = (
  changeData: QtChangeData,
  orData: any,
  exFn: (res: IparseChildUpdateRef) => void
) => {
    ...
    if (Array.isArray(k)) {
      ...
      if (changeData.names.size && changeData.names.get(k)?.size == 1) {
        ...
        if (
          isObject(orUdata) &&
          isObject(newUdata) &&
          isArray(orUdata[namev]) &&
          isArray(newUdata[namev])
        ) {
