...
export interface QtULAPI extends QtBaseViewAPI {
  scrollToIndex(
    instance: string | Ref<QTIUL | undefined>,
    x: number,
    y: number,
    animated: boolean,
    duration: number,
...
export function createQtULAPI(viewAPI: QtBaseViewAPI): QtULAPI {
  ...
  function scrollToContentOffset(
    instance: string | Ref<QTIUL | undefined>,
    x: number,
    y: number,
    animated: boolean,
  ...
  function setTargetFocusChildPosition(
    instance: string | Ref<QTIUL | undefined>,
    position: number
  ): void {
    if (isString(instance)) {
