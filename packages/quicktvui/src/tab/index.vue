  ...
  setup(props, context) {
    ...
    function setPageState(pageIndex: number, state: QTTabPageState): void {
      ...
      if (pageData) {
        ...
        if (log.isLoggable(ESLogLevel.DEBUG)) {
          ...
          log.d(
            TAG,
            '---------设置数据-----设置状态----->>>>' + ' pageIndex:' + pageIndex + ' state:' + st
          )
        }
    ...
    function setPageData(pageIndex: number, tabPageData: QTTabPageData): void {
      ...
      if (log.isLoggable(ESLogLevel.DEBUG)) {
        log.d(
          TAG,
          '---------设置数据-----setPageData------>>>>' + ' pageIndex:' + pageIndex + ' data:',
          data
        )
      }
      //
    ...
    function dispatchUIFunctionOnChildNode(
      ...
      tabs.value?.dispatchUIFunctionOnChildNode(
        page,
        position,
        childIndex,
        listViewName,
    ...
    function onTabPageLoadData(e: QTTabEvent): void {
      ...
      if (log.isLoggable(ESLogLevel.DEBUG)) {
        log.d(
          TAG,
          '----------设置数据----onTabPageLoadData---加载数据开始---->>>>pageIndex:' + pageIndex
        )
      }
      ...
      if (log.isLoggable(ESLogLevel.DEBUG)) {
        log.d(
          TAG,
          '----------设置数据----onTabPageLoadData---加载数据结束---->>>>pageIndex:' + pageIndex
        )
      }
    }
    ...
    function emitOnLoadTabPageDataEvent(
      ...
      if (log.isLoggable(ESLogLevel.DEBUG)) {
        log.d(
          TAG,
          '--1--emitOnLoadTabPageDataEvent---设置数据--->>>>' +
            ' caller: ' +
            caller +
            ' pageIndex: ' +
            pageIndex +
      ...
      if (
        ...
        if (log.isLoggable(ESLogLevel.DEBUG)) {
          log.d(
            TAG,
            '--2--emitOnLoadTabPageDataEvent---设置数据--->>>>' +
              ' caller: ' +
              caller +
              ' pageIndex: ' +
              pageIndex +
    ...
    function onTabPageChanged(e: QTTabEvent): void {
      ...
      for (let i = 0; i < tabItemList.length; i++) {
        ...
        if (log.isLoggable(ESLogLevel.DEBUG)) {
          log.d(
            TAG,
            '------设置数据--调用setPageStateRecycled--START->>>>' +
              ' index:' +
              i +
              ' lastTabPageIndex:' +
              lastTabPageIndex +
        ...
        if (
          ...
          if (log.isLoggable(ESLogLevel.DEBUG)) {
            log.d(
              TAG,
              '------设置数据--调用setPageStateRecycled--END->>>>' +
                ' index:' +
                i +
                ' lastTabPageIndex:' +
                lastTabPageIndex +
    ...
    function onTabPageScrollToStart(evt) {
      ...
      if (log.isLoggable(ESLogLevel.DEBUG)) {
        log.d(TAG, '----------onTabPageScrollToStart------>>>>' + ' pageIndex:' + pageIndex)
      }
    }
    ...
    function onTabPageScrollToEnd(evt) {
      ...
      if (log.isLoggable(ESLogLevel.DEBUG)) {
        log.d(TAG, '----------onTabPageScrollToEnd------>>>>' + ' pageIndex:' + pageIndex)
      }
    }
    ...
    function onWaterfallScroll(offsetX: number, scrollY: number) {
      if (log.isLoggable(ESLogLevel.DEBUG)) {
        log.d(
          TAG,
          '----------onWaterfallScroll---->>>>' + ' offsetX:' + offsetX + ' scrollY:' + scrollY
        )
      }
